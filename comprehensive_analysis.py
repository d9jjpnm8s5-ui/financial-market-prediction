...
def main(preferred_tickers: List[str] = None):
    """Run all analyses

    preferred_tickers: optional list of tickers provided programmatically
    or via CLI. If None, user will be prompted to enter tickers interactively
    (comma-separated) or the default samples will be used.
    """
    ...
    # Determine tickers to compare: CLI/parameter > interactive input > defaults
    if preferred_tickers:
        # Normalize and validate provided tickers
        requested = [t.strip().upper() for t in preferred_tickers if t]
    else:
        try:
            inp = input(
                "Enter tickers to compare (comma-separated), or press Enter to use defaults: "
            ).strip()
        except EOFError:
            inp = ""

        if inp:
            requested = [t.strip().upper() for t in inp.split(",") if t.strip()]
        else:
            requested = available_sample

    # Validate requested tickers against available data
    validated = [t for t in requested if t in all_tickers]
    if not validated:
        print("No valid tickers provided or matched; using defaults.")
        validated = available_sample

    # Run comparison and analyses
    compare_stocks_side_by_side(validated, days=365)
...
def _parse_args():
    p = argparse.ArgumentParser(description="Comprehensive stock market analysis")
    p.add_argument(
        "-t",
        "--tickers",
        help="Comma-separated list of tickers to compare (e.g. RELIANCE,TCS,INFY)",
        type=str,
    )
    return p.parse_args()
...
if __name__ == "__main__":
    ...
    tickers_arg = None
    if args.tickers:
        tickers_arg = [t.strip().upper() for t in args.tickers.split(",") if t.strip()]

    main(preferred_tickers=tickers_arg)